...
def load_quantized(model_name):
    ...
    pt_path = None
    priority_name_list = [
        Path(f'{shared.model_dir}/{model_name}{hyphen}{shared.wbits}bit{group}{ext}')
        for ext in ['.safetensors', '.pt']
        for group in ([f'-{shared.groupsize}g', ''] if shared.groupsize > 0 else [''])
        for hyphen in ['-', f'/{model_name}-', '/']
    ]
    if shared.groupsize > 0:
        priority_name_list = [i for i in priority_name_list if str(shared.groupsize) in i.name] + [i for i in priority_name_list if str(shared.groupsize) not in i.name]
    ...
    model = load_quant(str(path_to_model), str(pt_path), shared.wbits, shared.groupsize, kernel_switch_threshold=threshold)
