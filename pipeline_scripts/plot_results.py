...
if data:
    ...
    if 'llm_baseline' not in all_configurations:
        ...
        # Add 'baseline' (human text) if it exists
        if 'baseline' in all_configurations and 'baseline' not in configurations:
            configurations.insert(0, 'baseline') # Put human text first
    ...
    # Filter configurations to only those in the mapping
    ...
    colors = ['#D3D3D3'] + ['#ADD8E6'] * (len(plot_labels) - 2) + ['#FF6347'] # Human, Others, All Three
    if 'All Three' not in plot_labels: # Adjust if 'All Three' isn't plotted
    ...
    for i, config_key in enumerate(plot_configs, start=1):
        ...
        y_pos = ylim[0] - (ylim[1] - ylim[0]) * 0.1  # 10% below min y
        ax.text(i, y_pos, f'PPLX +{med_perp:.2f}\nDet: {percentage_below:.1f}%', ha='center', va='top', fontsize=9)
    
    ax.legend()
    plt.grid(True)
    plt.tight_layout(rect=[0, 0.05, 1, 1]) # Adjust layout for annotations
